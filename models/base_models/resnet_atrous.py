...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, atrous=1, downsample=None):
        ...
        self.bn3 = SynchronizedBatchNorm2d(
            planes * self.expansion, momentum=bn_mom)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
#                      )
        ...
        self.layer2 = self._make_layer(
            block, 256, 128, layers[1], stride=stride_list[0])
        self.layer3 = self._make_layer(
            block, 512, 256, layers[2], stride=stride_list[1], atrous=16//os)
        self.layer4 = self._make_layer(block, 1024, 512, layers[3], stride=stride_list[2], atrous=[
                                       item*16//os for item in atrous])
    ...
    def _make_layer(self, block, inplanes, planes, blocks, stride=1, atrous=None):
        ...
        layers.append(block(inplanes, planes, stride=stride,
                      atrous=atrous[0], downsample=downsample))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(planes*block.expansion,
                          planes, stride=1, atrous=atrous[i]))
...
def resnet50_atrous(pretrained=True, os=16, **kwargs):
    ...
    if pretrained:
        ...
        model_dict.update(old_dict)
        model.load_state_dict(model_dict)
        print('load resnet50 as pretrained backbone from {}'.format(
            kwargs['backbone_dir']))
    return model
...
def resnet101_atrous(pretrained=True, os=16, **kwargs):
    ...
    model = ResNet_Atrous(Bottleneck, [3, 4, 23, 3], atrous=[
                          2, 2, 2], os=os, **kwargs)
    if pretrained:
        old_dict = model_zoo.load_url(
            model_urls['resnet101'], model_dir=kwargs['backbone_dir'])
        model_dict = model.state_dict()
        old_dict = {k: v for k, v in old_dict.items() if (k in model_dict)}
        model_dict.update(old_dict)
        model.load_state_dict(model_dict)
        print('load resnet101 as pretrained backbone from {}'.format(
            kwargs['backbone_dir']))
    return model
...
def resnet152_atrous(pretrained=True, os=16, **kwargs):
    ...
    model = ResNet_Atrous(Bottleneck, [3, 8, 36, 3], atrous=[
                          1, 2, 1], os=os, **kwargs)
    if pretrained:
        old_dict = model_zoo.load_url(
            model_urls['resnet152'], model_dir=kwargs['backbone_dir'])
        model_dict = model.state_dict()
        old_dict = {k: v for k, v in old_dict.items() if (k in model_dict)}
        model_dict.update(old_dict)
        model.load_state_dict(model_dict)
        print('load resnet152 as pretrained backbone from {}'.format(
            kwargs['backbone_dir']))
    return model
