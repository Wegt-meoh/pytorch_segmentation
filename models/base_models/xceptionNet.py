...
def xception(pretrained=True, os=16):
    ...
    if pretrained:
        ...
        old_dict = model_zoo.load_url(
            model_urls['xception'], model_dir=kwargs['backbone_dir'])
        for name, weights in old_dict.items():
            if 'pointwise' in name:
                old_dict[name] = weights.unsqueeze(-1).unsqueeze(-1)
        model_dict = model.state_dict()
        old_dict = {k: v for k, v in old_dict.items() if (
            'itr' not in k and 'tmp' not in k and 'track' not in k)}
        model_dict.update(old_dict)

        model.load_state_dict(model_dict)
        print('load xception as pretrained backbone from {}'.format(
            kwargs['backbone_dir']))
